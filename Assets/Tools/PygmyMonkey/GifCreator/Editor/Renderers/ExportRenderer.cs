...
namespace PygmyMonkey.GifCreator
{
	public class ExportRenderer
	{
				...
				using (new GUIUtils.GUIContents())
				{
					...
					using (new GUIUtils.GUIEnabled(isButtonEnabled))
					{
						EditorGUI.BeginChangeCheck();
						{
							...
							gifCreatorWindow.Parameters.quality = EditorGUILayout.IntSlider(new GUIContent("Quality", "Higer values mean better quality but slightly longer processing time. 85 is generally a good middleground value."), gifCreatorWindow.Parameters.quality, 1, 100);
						}
		...
		private IEnumerator uploadImage()
		{
			...
			UnityWebRequest webRequest = UnityWebRequest.Post("https://api.imgur.com/3/image/", form);
            webRequest.SetRequestHeader("Authorization", "Client-ID " + "01c02b71cfa8334");
			...
			else
			{
				...
				try
				{
					result = JsonUtility.FromJson<ImgurResult>(webRequest.downloadHandler.text);
				}
				catch (Exception)
				{
					throw new UnityException("Error parsing json from imgur. Please contact us!");
				}
